...
- **Nombres de país**: usa `useCountryName()` de [utils/countryName.ts](utils/countryName.ts), no `t('countries.XX')` a pelo (si falta la clave, `t()` devuelve la clave cruda). Toma la clave del locale si existe y si no `Intl.DisplayNames`: un país nuevo en `COUNTRIES` sale traducido sin tocar los 31 locales.
