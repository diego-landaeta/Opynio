...
const AdminEnterprisePage: React.FC = () => {
    ...
    const handleAddUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!addedUser) throw new Error(t('adminEnterprisePage.updateReturnedNothing'));
    ...
    const handleSaveLimits = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) throw error;
            if (!updatedUser) throw new Error(t('adminEnterprisePage.updateReturnedNothing'));
                ...
                <div className="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-md border dark:border-zinc-700">
                    ...
                    <form onSubmit={handleAddUser}>
                        <div className="relative mb-4">
                            <input type="search" value={addUserSearchTerm} onChange={(e) => { setAddUserSearchTerm(e.target.value); setUserToAdd(null); }} placeholder={t('adminEnterprisePage.searchUserPlaceholder')} aria-label={t('adminEnterprisePage.searchUserPlaceholder')} className="w-full p-3 border border-gray-300 dark:border-zinc-600 rounded-lg bg-transparent" />
                                ...
                                <ul className="absolute z-20 w-full mt-1 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                    {addUserSearchResults.map(user => (
                                        <li key={user.id} onClick={() => { setUserToAdd(user); setAddUserSearchResults([]); setAddUserSearchTerm(user.name); }} className="px-4 py-3 hover:bg-gray-100 dark:hover:bg-zinc-700 cursor-pointer">
                                            <p className="font-semibold">{user.name}{user.username && <span className="font-normal text-gray-500"> (@{user.username})</span>}</p><p className="text-xs text-gray-500">{user.email}</p>
                                        </li>
                                    ))}
                                </ul>
                            )}
                        ...
                        {userToAdd && (
                            <div className="p-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800/50 rounded-lg space-y-3 animate-fade-in-up">
                                <h3 className="font-semibold text-green-800 dark:text-green-200" dangerouslySetInnerHTML={{ __html: t('adminEnterprisePage.confirmAddUser', { userName: escapeHtml(userToAdd.name) }) }} />
                                ...
                                enterpriseUsers.map(user => (
                                    <div key={user.id} className="border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
                                        <div onClick={() => handleSelectUser(user)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-700/50">
                                            ...
                                            </div><div><p className="font-semibold text-gray-900 dark:text-gray-100">{user.name}</p>{user.username && <p className="text-xs text-gray-500 dark:text-gray-400">@{user.username}</p>}</div></div>
                                            <div className="flex items-center gap-4 text-sm"><div className="hidden sm:block"><span className="font-semibold">Tokens:</span> {user.ai_credit_limit?.toLocaleString('es-ES') || '—'} | <span className="font-semibold">Empresas:</span> {user.business_limit || '—'}</div><i className={`fa-solid fa-chevron-down transition-transform ${selectedUser?.id === user.id ? 'rotate-180' : ''}`}></i></div>
                                        </div>
