...
export const HorizontalCarouselPreview: React.FC<PreviewProps> = ({ business, theme, lang }) => {
    ...
    return (
        <div className={`opynio-widget ${themeClass}`}>
            <div
                className="opynio-horizontal-carousel-widget"
                onMouseEnter={() => { pausedRef.current = true; }}
                onMouseLeave={() => { pausedRef.current = false; }}
            >
                <div className="opynio-horizontal-carousel-wrapper">
                    <a
                        href="#"
                        className="opynio-horizontal-carousel-cta-link"
                        onClick={(e) => e.preventDefault()}
                    >
                        <div className="opynio-horizontal-carousel-cta">
                            <div className="opynio-horizontal-carousel-score">{score}</div>
                            <div className="opynio-horizontal-carousel-score-stars">
                                <StaticStarRating rating={business.avg_rating || 5} />
                            </div>
                            <div className="opynio-horizontal-carousel-count">
                                <strong>{reviewCount}</strong> {s.reviews}
                            </div>
                            <div className="opynio-horizontal-carousel-rating-label">{ratingText}</div>
                        </div>
                    </a>

                    <div className="opynio-horizontal-carousel-right">
                        <div className="opynio-horizontal-carousel-cards-area">
                            <button onClick={scrollPrev} className="opynio-horizontal-carousel-nav opynio-horizontal-carousel-nav-prev" type="button" aria-label="Anterior">
                            ...
                            <div className="opynio-horizontal-carousel-cards-container" role="region" aria-label="Reseñas" aria-live="polite" aria-roledescription="carrusel">
                                <div className="opynio-horizontal-carousel-track" ref={trackRef}>
                                    {duplicated.map((review, index) => (
                                        <a
                                            key={`${review.id}-${index}`}
                                            href="#"
                                            className="opynio-horizontal-carousel-card"
                                            onClick={(e) => e.preventDefault()}
                                            title={review.original_author_name}
                                        >
                                            <div className="opynio-avatar-placeholder opynio-horizontal-carousel-card-avatar">
                                                {review.original_author_name.charAt(0)}
                                            </div>
                                            <div className="opynio-horizontal-carousel-card-name">{review.original_author_name.split(' ')[0]}</div>
                                            <div className="opynio-horizontal-carousel-card-stars">
                            ...
                            <button onClick={scrollNext} className="opynio-horizontal-carousel-nav opynio-horizontal-carousel-nav-next" type="button" aria-label="Siguiente">
                                <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z" /></svg>
                            </button>
                        </div>
                        <div className="opynio-horizontal-carousel-footer">
                            <a
                                href="#"
                                className="opynio-horizontal-carousel-footer-btn"
                                onClick={(e) => e.preventDefault()}
                            >
